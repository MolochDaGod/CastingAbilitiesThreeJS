 ...
 * Never uses full mesh AABB (weapons inflate min.y → hip float).
...
export function placeRootFeetAt(root, terrainY, THREE) {
  ...
  root.traverse((n) => {
    ...
    if (!isPlayFootBone(n.name)) return;
    n.getWorldPosition(v);
    if (Number.isFinite(v.y) && v.y < minY) minY = v.y;
  });
  if (!Number.isFinite(minY) || minY === Infinity) {
    // Bone-only fallback (calf/foot) — never full mesh AABB (weapons inflate Y).
    root.traverse((n) => {
      if (!n.isBone || !n.name) return;
      if (!/foot|toe|calf/i.test(n.name)) return;
      if (/hand|weapon|container|nub/i.test(n.name)) return;
      n.getWorldPosition(v);
      if (Number.isFinite(v.y) && v.y < minY) minY = v.y;
    });
  }
  if (!Number.isFinite(minY) || minY === Infinity) return 0;
  ...
  if (Math.abs(dy) > 1e-4) {
    ...
    root.matrix.decompose(pos, quat, scl);
    pos.y += dy;
    local.compose(pos, quat, scl);
    root.matrix.copy(local);
  ...
  feet.y = terrainY;
  root.userData.feetWorld = feet;
  root.userData.plantMatrix = root.matrix.clone();
  return dy;
}
